...
-   `netsploit-py` is a CLI wrapper for popular network security/pentesting tools including `nmap`, `hping3` and `ping`.
...
-   OR, you can Download the repository `.zip` file by clicking on `Code > Download ZIP` on this page. Note that using this method means you **cannot** update `netsploit-py` since when you last downloaded it.
...
1. Run the program (`netsploit-py`):
